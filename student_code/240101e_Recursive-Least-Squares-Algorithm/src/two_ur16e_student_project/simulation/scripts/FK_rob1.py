...
# Calculation of the current TCP-position and -orientation of Robot1
class GetFK(object):
    def __init__(self, fk_link, frame_id):
       
        self.fk_link = fk_link
        self.frame_id = frame_id
        self.fk_srv = rospy.ServiceProxy('/robot11/compute_fk', GetPositionFK)
        self.fk_srv.wait_for_service()
        self.last_js = None
        self.data_to_send = Float64MultiArray()
        self.js_sub = rospy.Subscriber('/robot1/joint_states', JointState, self.js_cb, queue_size=10)
        self.pub_TCP_robot1 = rospy.Publisher("/robot1/TCP_robot1", Float64MultiArray, queue_size=10)
    ...
    def tcpcalculate(self, resp):
        ...
        br.sendTransform((x, y, z), (quaternion[0], quaternion[1], quaternion[2], quaternion[3]), rospy.Time.now(), "robot11_tf/tool0", "robot11_tf/base_link")
        self.data_to_send.data = tcp
        self.pub_TCP_robot1.publish(self.data_to_send) 
        print(tcp)
        return tcp
