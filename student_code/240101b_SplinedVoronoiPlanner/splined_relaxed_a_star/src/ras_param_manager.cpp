#include <splined_relaxed_a_star/ras_param_manager.h>

namespace fpp_data_classes
{
RASParamManager::RASParamManager(ros::NodeHandle& nh, ros::NodeHandle& planner_nh) : nh_(nh), planner_nh_(planner_nh)
{
    // this->formation_info_srv_client_ =
    //     this->nh_.serviceClient<fpp_msgs::FormationFootprintInfo>("move_base_flex/footprint_info");
...
void RASParamManager::readParams(std::string formation_planner_name)
{
    ...
    if (this->planner_nh_.searchParam("formation_path_planner/" + formation_planner_name, path_planner_key))
    {
        if (!this->planner_nh_.getParam(path_planner_key + "/default_tolerance", this->ras_params_->default_tolerance))
        {
            this->reportParamError("default_tolerance");
        }
        int neighbor_type;
        if (!this->planner_nh_.getParam(path_planner_key + "/neighbor_type", neighbor_type))
        {
        ...
        // Minimal radius is calculated by getting the distance between the robot that is the furthest away from the
        // formation centre float max_calculated_minimal_curve_radius = 0.0;
        // for(std::shared_ptr<fpp_data_classes::RobotInfo> &robot_info: this->fpp_params_->getRobotInfoList())
        // {
        // 	float calculated_minimal_curve_radius =
        // this->target_formation_contour_.getRobotPosGeometryCS(robot_info->robot_name).norm();
    ...
    this->ras_params_->max_robot_to_formation_centre_dist = 1.0;
    XmlRpc::XmlRpcValue formation_robots;
    if(this->planner_nh_.getParam("formation_config/robot_names", formation_robots))
    {
        ...
        for (int i = 0; i < formation_robots.size(); i++)
        {
            ...
            this->planner_nh_.param("formation_config" + robot_namespace + "/rel_x_offset", rel_x_offset, rel_x_offset);
            this->planner_nh_.param("formation_config" + robot_namespace + "/rel_y_offset", rel_y_offset, rel_y_offset);
            double distance = sqrt(pow(rel_x_offset, 2) + pow(rel_y_offset, 2));
    ...
    {
        ROS_WARN("No Param for robot formation found, using default value");
    }
...
void RASParamManager::printInfo()
{
    ...
    ROS_INFO_STREAM(
        "Max Robot to Formation Centre Distance: " << this->ras_params_->max_robot_to_formation_centre_dist);
