...
BaseBezierSpline::BaseBezierSpline(int bezier_degree, visualization_helper::VisualizationHelper* visu_helper)
...
BaseBezierSpline::BaseBezierSpline(int bezier_degree, Eigen::Vector2f start_pose, Eigen::Vector2f end_pose)
    : BaseBezierSpline(bezier_degree)
{
...
BaseBezierSpline::BaseBezierSpline(int bezier_degree, visualization_helper::VisualizationHelper* visu_helper,
                                   Eigen::Vector2f start_pose, Eigen::Vector2f end_pose)
...
BaseBezierSpline::BaseBezierSpline(int bezier_degree, Eigen::Vector2f start_pose, Eigen::Vector2f start_tangent,
                                   float start_tangent_magnitude, Eigen::Vector2f end_pose, Eigen::Vector2f end_tangent,
...
BaseBezierSpline::BaseBezierSpline(int bezier_degree, visualization_helper::VisualizationHelper* visu_helper,
                                   Eigen::Vector2f start_pose, Eigen::Vector2f start_tangent,
                                   float start_tangent_magnitude, Eigen::Vector2f end_pose, Eigen::Vector2f end_tangent,
                                   float end_tangent_magnitude)
    : BaseBezierSpline(bezier_degree, start_pose, start_tangent, start_tangent_magnitude, end_pose, end_tangent,
...
void BaseBezierSpline::setEndTangentByNextPose(Eigen::Vector2f next_pose)
{
    ...
    float length_diff_vector_end_to_next = diff_vector_end_to_next.norm();  // Use length of end point and next point to
...
std::vector<Eigen::Vector2f> BaseBezierSpline::calcBezierSpline(int resolution)
{
    ...
    if (this->next_spline_ == nullptr)  // Add point at 1.0 because there is no spline after this that can add the point
                                        // thorugh its 0.0 value
    {
...
float BaseBezierSpline::calcSplineLength(float lower_bound, float upper_bound, float max_iterator_step_size)
{
    ...
    {
        // Eigen::Vector2f lower_point = this->calcPointOnBezierSpline(lower_bound + float(step_counter) *
        // real_step_size); Eigen::Vector2f upper_point = this->calcPointOnBezierSpline(lower_bound + float(step_counter
        // + 1) * real_step_size);

        // Eigen::Vector2f diff = upper_point - lower_point;
        // approx_spline_length = approx_spline_length + diff.norm();

        Eigen::Vector2f gradient_value =
            this->calcFirstDerivativeValue(lower_bound + float(step_counter) * real_step_size);
...
bool BaseBezierSpline::calcIteratorBySplineLength(float& iterator, float target_spline_length,
                                                  float max_diff_from_target, float max_iterator_step_size,
                                                  float& spline_length_remainder)
{
    return this->calcIteratorBySplineLength(iterator, target_spline_length, max_diff_from_target, 0.0,
                                            max_iterator_step_size, spline_length_remainder);
}

bool BaseBezierSpline::calcIteratorBySplineLength(float& iterator, float target_spline_length,
                                                  float max_diff_from_target, float first_step_size,
                                                  float max_iterator_step_size, float& spline_length_remainder)
{
...
float BaseBezierSpline::calcCurvation(float iterator)
{
    ...
    // ROS_INFO_STREAM("first derivative: " << first_derivative_value[0] << "|" << first_derivative_value[1] << " second
    // derivative: " << second_derivative_value[0] << "|" << second_derivative_value[1]); Calculate curvation. See
    // Calculus Early Transcendentals p. 856  for formula
...
void BaseBezierSpline::addStartEndPointToVisuHelper()
{
    ...
    this->visu_helper_->addMarkerToExistingMarkerArray(
...
void BaseBezierSpline::addBezierSplineToVisuHelper(int resolution)
{
    ...
    this->visu_helper_->addMarkerLineToMarkerArray(this->bezier_spline_identificator_, line,
...
void BaseBezierSpline::printInfo()
{
    ...
    for (int control_point_counter = 1; control_point_counter < (this->control_points_.size() - 1);
         control_point_counter++)
    {
        ROS_INFO_STREAM("ControlPoint" << control_point_counter << ": "
                                       << this->control_points_[control_point_counter][0] << " | "
                                       << this->control_points_[control_point_counter][1]);
    }
...
void BaseBezierSpline::initVisuHelper()
{
    ...
    this->initVisuHelper("start_end", "control_points", "tangents", "bezier_spline", "bezier_debug");
}
...
void BaseBezierSpline::initVisuHelper(std::string start_end_marker_identificator,
                                      ...
                                      std::string tangent_marker_identificator, std::string bezier_spline_identificator,
...
void BaseBezierSpline::addTangentToVisuHelper(Eigen::Vector2f start_point, Eigen::Vector2f tangent)
{
    if (this->isVisuHelperNull())
    {
        return;
    ...
    this->visu_helper_->addMarkerLineToMarkerArray(this->tangent_marker_identificator_, line,
                                                   this->tangent_marker_identificator_);
}

void BaseBezierSpline::addDebugVectorToVisuHelper(Eigen::Vector2f start_point, Eigen::Vector2f vector)
{
    if (this->isVisuHelperNull())
    {
        return;
    ...
    line.push_back(this->visu_helper_->createGeometryPoint(start_point[0], start_point[1]));
    line.push_back(this->visu_helper_->createGeometryPoint(end_point[0], end_point[1]));
    this->visu_helper_->addMarkerLineToMarkerArray(this->debug_marker_identificator_, line,
