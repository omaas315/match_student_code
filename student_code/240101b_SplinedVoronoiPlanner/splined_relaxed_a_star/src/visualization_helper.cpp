...
bool VisualizationHelper::visualizeMarkerArrays()
{
    ...
    {
        ROS_ERROR(
            "VisualizationHelper: No marker arrays to be visualized. Please add marker arrays and markers before "
            "visualizing.");
        return false;
    }
}
...
bool VisualizationHelper::visualizeMarkerArray(std::string marker_array_identifier)
{
    ...
    {
        ROS_ERROR(
            "VisualizationHelper: identifier of marker_array was not found. Please check the given identifiers that "
            "were handed to this method.");
        return false;
    }
}
...
std::string VisualizationHelper::addNewMarkerArray()
{
    ...
    {
        ROS_ERROR_STREAM("VisualizationHelper: Something went wrong when adding an unnamed marker array with the index " <<
...
bool VisualizationHelper::addNewMarkerArray(std::string marker_array_identifier)
{
    ...
    {
        ROS_ERROR_STREAM(
            "VisualizationHelper: Marker array identifier was already existing. Error while inserting new marker array "
            "with indentificator: " << marker_array_identifier);
        return false;
    }
}
...
std::string VisualizationHelper::addMarkerTemplate(visualization_msgs::Marker marker_template)
{
    ...
    {
        ROS_ERROR_STREAM("VisualizationHelper: Something went wrong when adding an unnamed marker templare with the index " <<
                  this->marker_template_list_.size() - 1);
        return "";
    }
}

bool VisualizationHelper::addMarkerTemplate(std::string marker_template_identifier,
                                            visualization_msgs::Marker marker_template)
{
    ...
    else
    {
        ROS_ERROR_STREAM(
            "VisualizationHelper: Marker array identifier was already existing. Error while inserting new marker array "
            "with indentificator: " << marker_template_identifier);
        return false;
    }
}
...
bool VisualizationHelper::addMarkerToExistingMarkerArray(std::string marker_array_identifier, geometry_msgs::Pose pose,
...
bool VisualizationHelper::clearAllMarkerArrays()
{
    for (auto marker_array: this->marker_array_list_)
    {
        // ROS_INFO_STREAM("Clearing Marker: " << marker_array.first);
        this->marker_array_list_[marker_array.first].markers.clear();
    }
    return true;
}
...
void VisualizationHelper::setTimeStamps(std::string marker_array_identifier)
{
    if (this->marker_array_list_[marker_array_identifier].markers.size() != 0)
    {
        for (int marker_counter = 0;
             marker_counter < (this->marker_array_list_[marker_array_identifier].markers.size() - 1); marker_counter++)
        {
