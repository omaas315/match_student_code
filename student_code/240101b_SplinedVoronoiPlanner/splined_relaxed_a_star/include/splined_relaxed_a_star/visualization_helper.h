...
public:
     ...
     *
     * @return string Index of the marker array that was added so the external code can add marker directly to the
     * marker array
     */
     ...
     */
    bool addMarkerToExistingMarkerArray(std::string marker_array_identifier, visualization_msgs::Marker marker_to_add);
    ...
    bool addMarkerToExistingMarkerArray(std::string marker_array_identifier, geometry_msgs::Pose pose,
                                        std::string marker_template_identifier);

    bool addMarkerLineToMarkerArray(std::string marker_array_identifier, std::vector<geometry_msgs::Point> points,
                                    std::string marker_template_identifier);
    ...
    /**
     * @brief Clear all marker arrays to not publish points from previous runs again
     *
     * @return true Marker array exists and was cleared
     */
    bool clearAllMarkerArrays();
...
private:
     ...
     * The first marker array in the list will be published first and might be overwritten by other markers in higher
     * lists.
     *
     */
     ...
     * This makes adding new markers really easy as only the position must be set and a template selected (that was
