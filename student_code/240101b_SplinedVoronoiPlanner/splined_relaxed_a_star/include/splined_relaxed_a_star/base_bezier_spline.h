...
#pragma region Constructors
    ...
    BaseBezierSpline(int bezier_degree, visualization_helper::VisualizationHelper* visu_helper);
    BaseBezierSpline(int bezier_degree, Eigen::Vector2f start_pose, Eigen::Vector2f end_pose);
    BaseBezierSpline(int bezier_degree, visualization_helper::VisualizationHelper* visu_helper,
                     Eigen::Vector2f start_pose, Eigen::Vector2f end_pose);
    BaseBezierSpline(int bezier_degree, Eigen::Vector2f start_pose, Eigen::Vector2f start_tangent,
                     float start_tangent_magnitude, Eigen::Vector2f end_pose, Eigen::Vector2f end_tangent,
                     float end_tangent_magnitude);
    BaseBezierSpline(int bezier_degree, visualization_helper::VisualizationHelper* visu_helper,
                     Eigen::Vector2f start_pose, Eigen::Vector2f start_tangent, float start_tangent_magnitude,
...
#pragma region BezierMethods
     ...
     *
     * @param iterator Input: Starting point from where to measure the spline length. Output: Where the
     * target_spline_length is reached
     * @param target_spline_length Method should return iterator that is target_spline_length away from the point where
     * the iterator starts
     ...
     * target_spline_length is reached the target_spline_length is excactly between iterator and the 1.0 end.
     * @return true target_spline_length was reached before iterator = 1.0
     * @return false
     */
    virtual bool calcIteratorBySplineLength(float& iterator, float target_spline_length, float max_diff_from_target,
                                            float max_step_size, float& spline_length_remainder);
     ...
     * the iterator starts
     * @param max_diff_from_target Max difference between the calculated spline length and the target spline length
     * @param first_step_size This defines the first step size the iterator will make. After that the max_step_size will
     * be used.
     ...
     */
    virtual bool calcIteratorBySplineLength(float& iterator, float target_spline_length, float max_diff_from_target,
                                            float first_step_size, float max_step_size, float& spline_length_remainder);
...
#pragma region PrivateVisuHelper
    void initVisuHelper();
    void initVisuHelper(std::string start_end_marker_identificator, std::string control_point_marker_identificator,
