...
{
CubicBezierSplines::CubicBezierSplines() : BaseBezierSpline(3)
{
}
...
CubicBezierSplines::CubicBezierSplines(Eigen::Vector2f start_pose, Eigen::Vector2f end_pose)
...
CubicBezierSplines::CubicBezierSplines(Eigen::Vector2f start_pose, Eigen::Vector2f start_tangent,
                                       float start_tangent_magnitude, Eigen::Vector2f end_pose,
                                       Eigen::Vector2f end_tangent, float end_tangent_magnitude)
    : BaseBezierSpline(3, start_pose, start_tangent, start_tangent_magnitude, end_pose, end_tangent,
...
CubicBezierSplines::CubicBezierSplines(visualization_helper::VisualizationHelper* visu_helper,
                                       ...
                                       float start_tangent_magnitude, Eigen::Vector2f end_pose,
                                       Eigen::Vector2f end_tangent, float end_tangent_magnitude)
    : BaseBezierSpline(3, visu_helper, start_pose, start_tangent, start_tangent_magnitude, end_pose, end_tangent,
                       end_tangent_magnitude)
{
}
...
void CubicBezierSplines::calcControlPoints()
{
    ...
    this->control_points_[1] = (1.0 / 3.0) * this->getMultipliedStartTangent() + this->control_points_.front();
    this->control_points_[2] = -(1.0 / 3.0) * this->getMultipliedEndTangent() + this->control_points_.back();
}
...
Eigen::Vector2f CubicBezierSplines::calcFirstDerivativeValue(float iterator)
{
    ...
    first_derivative_value =
        3 * std::pow(1 - iterator, 2) * (this->control_points_[1] - this->control_points_.front()) +
...
Eigen::Vector2f CubicBezierSplines::calcSecondDerivativeValue(float iterator)
{
    ...
    second_derivative_value =
        6 * (1 - iterator) * (this->control_points_.front() - 2 * this->control_points_[1] + this->control_points_[2]) +
        6 * iterator * (this->control_points_[1] - 2 * this->control_points_[2] + this->control_points_.back());

    // ROS_INFO_STREAM("Iterator: " << iterator << " Second derivative value:" << second_derivative_value[0] << " | " <<
    // second_derivative_value[1] << " Start:" << this->start_pose_[0] << " | " << this->start_pose_[1] << " cp1: " <<
    // this->cp1_[0] << " | " << this->cp1_[1] << " cp2: " << this->cp2_[0] << " | " << this->cp2_[1] << " end: " <<
    // this->end_pose_[0] << " | " << this->end_pose_[1] << " end tangente: " << this->end_tangent_[0] << " | " <<
    // this->end_tangent_[1]);
