#include <splined_relaxed_a_star/splined_relaxed_a_star.h>
#include <pluginlib/class_list_macros.h>

// register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(path_planner::SplinedRelaxedAStar, nav_core::BaseGlobalPlanner)

namespace path_planner
{

SplinedRelaxedAStar::SplinedRelaxedAStar() : initialized_(false)
{
    ROS_ERROR("RELAXED A STAR DEFAULT CONSTRUCTOR");
}

SplinedRelaxedAStar::SplinedRelaxedAStar(std::string name, costmap_2d::Costmap2DROS* costmap_ros) : initialized_(false)
{
...
void SplinedRelaxedAStar::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
    if (!this->initialized_)
    {
        ...
        this->planner_nh_ = ros::NodeHandle("~/" + name);
        this->ras_param_manager_ = std::make_shared<fpp_data_classes::RASParamManager>(this->nh_, this->planner_nh_);
        this->ras_param_manager_->readParams("SplinedRelaxedAStar");
        ...
        this->make_plan_service_ = private_nh.advertiseService("make_plan", &SplinedRelaxedAStar::makePlanService, this);
...
bool SplinedRelaxedAStar::makePlanService(navfn::MakeNavPlan::Request& req, navfn::MakeNavPlan::Response& res)
{
...
bool SplinedRelaxedAStar::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
                                       ...
                                       std::string& message)
{
    ...
    bool curvature_success = true;
    this->visu_helper_.clearAllMarkerArrays();
    ...
    if (g_score[array_goal_cell] != std::numeric_limits<float>::infinity())
    {
        ...
        if (!curvature_success)
        {
            ROS_ERROR("Plan doesn't fulfil curvature constraint");
            return 54;
        }
        ...
        for (std::shared_ptr<bezier_splines::QuinticBezierSplines>& spline : spline_list)
        {
            ...
            /*
            for(int iterator_counter = 0; iterator_counter < this->ras_params_->planning_points_per_spline; iterator_counter++)
            {
                iterator = (float)iterator_counter / (float)this->ras_params_->planning_points_per_spline;
                Eigen::Vector2f point = spline->calcPointOnBezierSpline(iterator);
                points_of_plan.push_back(point);
            }
            */
            do
            {
                ...
                if (spline_not_ended)
                {
                    ...
                    points_of_plan.push_back(point_on_spline);
                }
    ...
    ROS_INFO_STREAM("Planning finished " << plan.size());
...
std::vector<int> SplinedRelaxedAStar::createPlan(int array_start_cell, int array_goal_cell,
...
void SplinedRelaxedAStar::createPoseArrayForPlan(std::vector<int> array_plan,
                                                 std::vector<geometry_msgs::PoseStamped>& plan)
{
    geometry_msgs::PoseStamped last_pose;
    // < is necessary because we just copy elements from one vector (0 until size()) to the other
...
void SplinedRelaxedAStar::createOccupancyMap()
{
    ...
    // move costmap to opencv image
    // compute distance image
    // threshold by formation radius
    double costmap_size_x = costmap_->getSizeInCellsX();
    double costmap_size_y = costmap_->getSizeInCellsY();
    ...
    for (int i = 0; i < costmap_size_x; i++)
    {
        for (int j = 0; j < costmap_size_y; j++)
        {
            ...
            if (cell_cost > this->ras_params_->free_cell_threshold)
            {
    ...
    cv::threshold(costmap_img, costmap_img, this->ras_params_->max_robot_to_formation_centre_dist / this->costmap_->getResolution(), 255, cv::THRESH_BINARY_INV);
    costmap_img.convertTo(costmap_img, CV_8UC1);
    cv::imwrite("/home/soenke/Bilder/costmap_img.png", costmap_img);
    ...
    for (int cell_counter_x = 0; cell_counter_x < this->costmap_->getSizeInCellsX(); cell_counter_x++)
    {
        for (int cell_counter_y = 0; cell_counter_y < this->costmap_->getSizeInCellsY(); cell_counter_y++)
        {
            ...
            unsigned short int cell_cost = costmap_img.at<uchar>(cell_counter_x, cell_counter_y, 0);
...
float SplinedRelaxedAStar::calcMoveCost(int map_current_cell_x, int map_current_cell_y, int map_target_cell_x,
...
float SplinedRelaxedAStar::calcFCost(float current_cell_g_score, int array_current_cell, int array_target_cell,
                                     int array_goal_cell)
{
...
void SplinedRelaxedAStar::createMarkersForGScoreArray(std::shared_ptr<float[]> g_score)
{
    for (int counter = 0; counter < this->array_size_; counter++)
    {
        if (g_score[counter] != std::numeric_limits<float>::infinity())
        {
            ...
            pose.position.z = 0.0;
            this->visu_helper_.addMarkerToExistingMarkerArray(this->g_score_marker_identificator_, pose,
...
SplinedRelaxedAStar::createPoseArrayForOrientationVisu(std::vector<geometry_msgs::PoseStamped>& plan)
{
    ...
    {
        planning_points_orientation.poses.insert(
            planning_points_orientation.poses.begin() + planning_points_orientation.poses.size(), pose_stamped.pose);
    }
...
void SplinedRelaxedAStar::initVisuHelper(std::string topic_prefix, std::string g_score_marker_identificator,
