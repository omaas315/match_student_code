#include <splined_relaxed_a_star/quintic_bezier_spline.h>

namespace bezier_splines
{
QuinticBezierSplines::QuinticBezierSplines() : BaseBezierSpline(5)
{
}
...
QuinticBezierSplines::QuinticBezierSplines(Eigen::Vector2f start_pose, Eigen::Vector2f end_pose)
...
QuinticBezierSplines::QuinticBezierSplines(Eigen::Vector2f start_pose, Eigen::Vector2f start_tangent,
                                           float start_tangent_magnitude, Eigen::Vector2f end_pose,
                                           Eigen::Vector2f end_tangent, float end_tangent_magnitude)
    : BaseBezierSpline(5, start_pose, start_tangent, start_tangent_magnitude, end_pose, end_tangent,
...
QuinticBezierSplines::QuinticBezierSplines(visualization_helper::VisualizationHelper* visu_helper,
                                           ...
                                           float start_tangent_magnitude, Eigen::Vector2f end_pose,
                                           Eigen::Vector2f end_tangent, float end_tangent_magnitude)
    : BaseBezierSpline(5, visu_helper, start_pose, start_tangent, start_tangent_magnitude, end_pose, end_tangent,
                       end_tangent_magnitude)
{
}
...
void QuinticBezierSplines::calcControlPoints()
{
    ...
    if (this->previous_spline_ != nullptr)
    {
        ...
        previous_spline->calcControlPoints();  // Everything is set for the previous spline. Control points can be
                                               // calculated
    ...
    if (previous_spline != nullptr)  // Calc second derivative value through average of both connecting splines
    {
        ...
        average_start_second_derivative_val =
            0.5 * (prev_spline_second_derivative_val + curr_spline_second_derivative_start_val);
    }
    ...
    this->control_points_[2] = (1.0 / 20.0) * average_start_second_derivative_val + 2.0 * this->control_points_[1] -
    ...
    if (next_spline != nullptr)  // Calc second derivative value through average of both connecting splines
    {
        ...
        average_end_second_derivative_val =
            0.5 * (next_spline_second_derivative_val + curr_spline_second_derivative_end_val);
        // ROS_INFO_STREAM("average: " << average_end_second_derivative_val[0] << " | " <<
        // average_end_second_derivative_val[1] << " next: " << next_spline_second_derivative_val[0] << " | " <<
        // next_spline_second_derivative_val[1] << " curr: " << curr_spline_second_derivative_end_val[0] << " | " <<
        // curr_spline_second_derivative_end_val[1]);
    }
    ...
    this->control_points_[3] = (1.0 / 20.0) * average_end_second_derivative_val + 2.0 * this->control_points_[4] -
                               this->control_points_.back();
    // ROS_INFO_STREAM("cp3_: " << cp3_[0] << " | " << cp3_[1] << "cp4_: " << cp4_[0] << " | " << cp4_[1] << "end_pose_:
    // " << end_pose_[0] << " | " << end_pose_[1]);
}
...
Eigen::Vector2f QuinticBezierSplines::calcSecondDerivativeValue(float iterator)
{
    Eigen::Vector2f second_derivative_value =
        20 * std::pow(1 - iterator, 3) *
            (this->control_points_.front() - 2 * this->control_points_[1] + this->control_points_[2]) +
        60 * std::pow(1 - iterator, 2) * iterator *
            (this->control_points_[1] - 2 * this->control_points_[2] + this->control_points_[3]) +
        60 * (1 - iterator) * std::pow(iterator, 2) *
            (this->control_points_[2] - 2 * this->control_points_[3] + this->control_points_[4]) +
        20 * std::pow(iterator, 3) *
            (this->control_points_[3] - 2 * this->control_points_[4] + this->control_points_.back());
...
QuinticBezierSplines::createCubicBezierSpline(std::shared_ptr<bezier_splines::BaseBezierSpline> base_spline)
{
    std::shared_ptr<bezier_splines::CubicBezierSplines> cubic_spline =
        std::make_shared<bezier_splines::CubicBezierSplines>(
            base_spline->getStartPose(), base_spline->getStartTangent(), base_spline->getStartTangentMagnitude(),
            base_spline->getEndPose(), base_spline->getEndTangent(), base_spline->getEndTangentMagnitude());
