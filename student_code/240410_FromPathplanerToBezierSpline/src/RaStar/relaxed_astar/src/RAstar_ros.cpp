...
ros::Publisher path_publisher; //added
...
void RAstarPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
  ...
  if (!initialized_)
  {
    ...
    initialized_ = true;
    path_publisher = private_nh.advertise<nav_msgs::Path>("global_plan", 1, true);
    ROS_INFO("Path publisher created");
  }
  else
...
MyExcelFile << startCell <<"\t"<< start.pose.position.x <<"\t"<< start.pose.position.y <<"\t"<< goalCell <<"\t"<< goal.pose.position.x <<"\t"<< goal.pose.position.y;
  ...
  if (isStartAndGoalCellsValid(startCell, goalCell)){
	...
	MyExcelFile << "\t" <<path_length <<"\t"<< plan.size() <<endl;
      ...
      // Kopieren Sie die Poses in die 'nav_msgs::Path' Nachricht
      for (int i = 0; i < bestPath.size(); i++)
      {
          float x = 0.0;
          float y = 0.0;
          ...
          pose.header.frame_id = "map"; // or whatever frame you're operating in
          pose.pose.position.x = x;
          pose.pose.position.y = y;
          pose.pose.position.z = 0.0;
          pose.pose.orientation = tf::createQuaternionMsgFromYaw(0); // or your specific orientation
