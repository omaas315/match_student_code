...
namespace bezier_splines
{
	...
	BaseBezierSpline::BaseBezierSpline(int bezier_degree) // 1
		: visu_helper_(nullptr),
		  BEZIER_DEGREE(bezier_degree),
		  previous_spline_(nullptr),
		  next_spline_(nullptr)
	{
		this->initControlPointList(); // fills with Eigen::Vector::Zeros()
	}

	BaseBezierSpline::BaseBezierSpline(int bezier_degree,
									   visualization_helper::VisualizationHelper *visu_helper) // 2
	...
	BaseBezierSpline::BaseBezierSpline(int bezier_degree,
									   ...
									   Eigen::Vector2f start_pose,
									   Eigen::Vector2f end_pose) // 4
		: BaseBezierSpline(bezier_degree, waypoints, start_pose, end_pose)
	{
		this->visu_helper_ = visu_helper;
		this->initVisuHelper();
	}
	...
	BaseBezierSpline::BaseBezierSpline(int bezier_degree,
									   ...
									   Eigen::Vector2f goalTangent) // 5
		: BaseBezierSpline(bezier_degree, waypoints, start_pose, end_pose)
	{
		this->waypoints_ = waypoints; // added
		this->j_ = j; //added
		this->start_tangent_ = start_tangent;
		this->end_tangent_ = end_tangent;
		this->goal_tangent_ = goalTangent; // added
	}
	...
	std::vector<Eigen::Vector2f> BaseBezierSpline::getWaypoints()
    {
        return this->waypoints_;
    }

	void BaseBezierSpline::setWaypoints(std::vector<Eigen::Vector2f> waypoints)
    {
        this->waypoints_ = waypoints;
    }

	size_t BaseBezierSpline::getJ()
	{
		return this -> j_;
	}

	void BaseBezierSpline::setJ(size_t j)
	{
		this->j_ = j;
	}
	...
	void BaseBezierSpline::setStartPose(Eigen::Vector2f start_pose)
	{
		this->control_points_.front() = start_pose;
	}
	...
	void BaseBezierSpline::setEndPose(Eigen::Vector2f end_pose)
	{
		this->control_points_.back() = end_pose;
	}
	...
	Eigen::Vector2f BaseBezierSpline::getGoalTangent()
	{
		return this->goal_tangent_;
	}
	void BaseBezierSpline::setGoalTangent(Eigen::Vector2f goalTangent)
	{
		this->goal_tangent_ = goalTangent;
	}
	#pragma endregion
	...
	void BaseBezierSpline::updatePreviousSpline(const std::shared_ptr<BaseBezierSpline> &previous_spline)
	{	
		this->previous_spline_ = previous_spline->clone();

	}

	void BaseBezierSpline::updateCurrentSpline(const std::shared_ptr<BaseBezierSpline> &current_spline)
	{
		this->setStartPose(current_spline->getStartPose());
		this->setEndPose(current_spline->getEndPose());
		this->setStartTangent(current_spline->getStartTangent());
		this->setEndTangent(current_spline->getEndTangent());
	}


	// void BaseBezierSpline::setMemberCurrentSpline(const std::shared_ptr<BaseBezierSpline> &current_spline) {
	// 	this->current_spline_ = current_spline;
	// 	//std::cout<<"curent_spline_ set: "<<current_spline_->getStartPose().x()<<", =? "<<current_spline->getStartPose().x()<<std::endl; // ja
    // }

	void BaseBezierSpline::setMemberNextSpline(const std::shared_ptr<BaseBezierSpline> &next_spline) {
		this->next_spline_ = next_spline;
		//std::cout<<"curent_spline_ set: "<<current_spline_->getStartPose().x()<<", =? "<<current_spline->getStartPose().x()<<std::endl; // ja		
    }

	void BaseBezierSpline::setMemberPreviousSpline(const std::shared_ptr<BaseBezierSpline> &previous_spline) {
		this->previous_spline_ = previous_spline;
		//std::cout<<"curent_spline_ set: "<<current_spline_->getStartPose().x()<<", =? "<<current_spline->getStartPose().x()<<std::endl; // ja
    }
	...
	const std::vector<Eigen::Vector2f>& BaseBezierSpline::getControlPoints() {
        return this->control_points_;
    }
	
	const std::vector<Eigen::Vector2f>& BaseBezierSpline::getPreviousControlPoints() {
        if (previous_spline_ != nullptr) {
            return previous_spline_->getControlPoints();
        } else {
            static const std::vector<Eigen::Vector2f> empty;
            return empty;
        }
    }

	const std::vector<Eigen::Vector2f>& BaseBezierSpline::getNextControlPoints() {
        if (next_spline_ != nullptr) {
            return next_spline_->getControlPoints();
        } else {
            static const std::vector<Eigen::Vector2f> empty;
            return empty;
        }
    ...
    void BaseBezierSpline::addTangentsToVisuHelper()
    {
        ...
        this->addTangentToVisuHelper(this->control_points_.front(), this->getStartTangent());
        this->addTangentToVisuHelper(this->control_points_.back(), this->getEndTangent());
    }
